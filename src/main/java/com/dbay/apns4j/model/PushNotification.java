...
public class PushNotification {
	...
	/**
	 * We need to check the size of payload. After checking, reuse it in order
	 * to avoid calculate bytes array twice although it seems not perfect
	 * 
	 * @param payloads
	 * @return
	 */
	public byte[] generateData(byte[] payloads) {
		...
		list.add(new FrameItem(FrameItem.ITEM_ID_NOTIFICATION_IDENTIFIER,
				ApnsTools.intToBytes(getId(), 4)));
		list.add(new FrameItem(FrameItem.ITEM_ID_EXPIRATION_DATE, ApnsTools
				.intToBytes(getExpire(), 4)));
		list.add(new FrameItem(FrameItem.ITEM_ID_PRIORITY, ApnsTools
				.intToBytes(getPriority(), 1)));
		// return ApnsTools.generateData(getId(), getExpire(), tokens,
		// payloads);
